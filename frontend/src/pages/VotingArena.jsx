...
const VotingArena = () => {
  ...
  useEffect(() => {
    const bootstrapVotingData = async () => {
      ...
      try {
        const [candidatesResponse, meResponse] = await Promise.all([
        ...
        setVoterName(meResponse.data.name);
        setHasVoted(meResponse.data.hasVoted);

        if (meResponse.data.hasVoted) {
          try {
        ...
        setError('Unable to load ballot data. Please refresh and try again.');
      } finally {
            ...
            <section>
              {Object.keys(groupedCandidates).length === 0 ? (
                <div className="surface-card p-8">
                  ...
                  <p className="text-[#5e7398]">Candidates have not been configured yet. Please check again shortly.</p>
                </div>
              ) : (
            ...
            <aside className="glass-panel p-6 h-fit lg:sticky lg:top-28">
              ...
              <div className="rounded-2xl border border-[#d2def6] bg-white p-4 mb-5">
                <p className="text-sm text-[#4f6691] leading-relaxed">
                  One account can cast only one vote. Review your selection carefully before submitting.
                </p>
              </div>
              ...
              <button
                ...
                disabled={loading || hasVoted || !selectedCandidate || !agreeToTerms}
                className="btn-primary w-full disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {loading ? 'Submitting vote...' : hasVoted ? 'Vote Already Submitted' : 'Confirm Vote'}
              </button>
                ...
                <div className="mt-3">
                  ...
                  {receipt && (
                    <div className="mt-3 rounded-2xl border border-[#bcd0f5] bg-[#f4f8ff] p-4">
                      ...
                      <Link to={`/receipt?code=${encodeURIComponent(receiptCode)}`} className="btn-secondary mt-3 !py-2 !px-4 text-xs inline-flex">
                        Verify This Receipt
                      </Link>
                    </div>
                  )}
